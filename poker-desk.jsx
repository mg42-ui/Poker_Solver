...
function createSim(hero, board, opponents, seed) {
  ...
  const pools = opponents.map((o) => {
    const p = rangeCombos(Math.max(0.02, Math.min(1, o.pct))).filter((c) => !dead[c[0]] && !dead[c[1]]);
    return p.length ? p : ALL_COMBOS.filter((c) => !dead[c[0]] && !dead[c[1]]);
  });
    ...
    while (done < iters && guard < iters * 40) {
      ...
      for (let o = 0; o < nOpp; o++) {
        ...
        for (let t = 0; t < 40; t++) {
          ...
          if (!used[c[0]] && !used[c[1]]) { picked = c; break; }
        }
      ...
      for (let o = 0; o < nOpp; o++) {
        const s = evaluate([oppHands[o][0], oppHands[o][1], ...runBoard]);
...
function rangeForOpponent(v, action, street) {
  ...
  let pct = s.vpip;
  pct *= ACTION_TIGHTEN[action] ?? 1;
  if (street > 0 && action !== "none") pct *= 0.88;
...
/* ============================================================
   CARD PICKER
   ============================================================ */
function Picker({ open, used, onPick, onClose, target }) {
  if (!open) return null;
  return (
    <div className="picker-wrap" onClick={onClose}>
      <div className="picker" onClick={(e) => e.stopPropagation()}>
        <div className="picker-head">
          <span>Choose a card for {target}</span>
          <button className="x" onClick={onClose}>close</button>
        </div>
        <div className="deck">
          {[1, 2, 3, 0].map((s) => (
            <div className="deck-row" key={s}>
              {Array.from({ length: 13 }, (_, i) => {
                const id = (12 - i) * 4 + s;
                const isUsed = used.includes(id);
                return (
                  <button key={id} disabled={isUsed}
                    className={`pcard ${s === 1 || s === 2 ? "red" : "black"} ${isUsed ? "used" : ""}`}
                    onClick={() => onPick(id)}>
                    {RANKS[cardRank(id)]}<i>{SUIT_GLYPH[s]}</i>
                  </button>
                );
              })}
            </div>
...
export default function PokerDesk() {
  ...
  useEffect(() => {
    setSeats((prev) => {
      ...
      while (next.length < want) next.push({ villainId: null, action: "none" });
      return next;
    });
  }, [nPlayers]);

  const boardActive = board.slice(0, STREETS[street].cards);
  const knownBoard = boardActive.filter((c) => c !== null);
  const usedCards = [...hole, ...board].filter((c) => c !== null);
  const heroReady = hole[0] !== null && hole[1] !== null;
  const boardReady = knownBoard.length === STREETS[street].cards;
  const ready = heroReady && boardReady;

  const opponents = useMemo(
    () => seats.map((s) => ({ pct: rangeForOpponent(villains.find((v) => v.id === s.villainId), s.action, street) })),
  ...
  const verdict = useMemo(() => {
    ...
    if (toCall <= 0) {
      ...
      if (strong) {
        ...
        action = eq < 0.2 && nPlayers === 2 ? "BLUFF OR CHECK" : "CHECK / FOLD";
        tone = "bad";
        size = eq < 0.2 && nPlayers === 2 ? potBefore * 0.6 : null;
        reasons.push(`Low equity (${(eq * 100).toFixed(1)}%). A 60%-pot bluff needs them to fold ${(fe * 100).toFixed(0)}% of the time.`);
        if (nPlayers > 2) reasons.push(`${nPlayers - 1} opponents — bluffs need everyone to fold, which compounds against you.`);
      }
    } else {
      if (edge > 0.15 && eq > 0.6) {
        action = "RAISE";
  ...
  }, [sim, eq, req, ev, toCall, pot, potBefore, texture, spr, street, prePct, preCode, preRank, position, bb, nPlayers]);
  ...
  const setCard = (id) => {
    if (!picker) return;
    if (picker.type === "hole") setHole((h) => h.map((c, i) => (i === picker.i ? id : c)));
    else setBoard((b) => b.map((c, i) => (i === picker.i ? id : c)));
    setPicker(null);
  };
  ...
  return (
    <div className="root">
      <style>{CSS}</style>
      <Picker open={!!picker} used={usedCards} target={picker?.label || ""}
        onPick={setCard} onClose={() => setPicker(null)} />
      ...
      {tab === "table" && (
        <main className="grid">
          ...
          <section className="panel">
            <div className="rail">
              {STREETS.map((s, i) => (
                <button key={s.key} className={`stop ${i === street ? "on" : ""} ${i < street ? "past" : ""}`}
                  onClick={() => setStreet(i)}>
            ...
            <div className="block">
              <div className="blabel">Your hand</div>
              <div className="cards">
                {hole.map((c, i) => (
                  <Card key={i} id={c} size="lg" onClick={() => setPicker({ type: "hole", i, label: `your card ${i + 1}` })} />
                ))}
                {preCode && (
                  <div className="handmeta">
                    <b>{preCode}</b>
                    <span>#{preRank} of 169 · top {(prePct * 100).toFixed(0)}%</span>
                  </div>
                )}
              </div>
            </div>
            ...
            {street > 0 && (
              <div className="block">
                <div className="blabel">Board</div>
                <div className="cards">
                  {Array.from({ length: STREETS[street].cards }, (_, i) => (
                    <Card key={i} id={board[i]} size="lg"
                      onClick={() => setPicker({ type: "board", i, label: `board card ${i + 1}` })} />
                  ))}
                </div>
                {texture && (
            ...
            <div className="block">
              <div className="blabel">Table</div>
              <div className="row2">
                <label className="numf">
                  <span>Players in the pot</span>
                  <div className="seg">
                    {[2, 3, 4, 5, 6, 7, 8, 9].map((n) => (
                      <button key={n} className={n === nPlayers ? "on" : ""} onClick={() => setNPlayers(n)}>{n}</button>
                    ))}
                  </div>
                </label>
                <label className="numf">
              ...
              <div className="seats">
                {seats.map((s, i) => {
                  const v = villains.find((x) => x.id === s.villainId);
                  const st = v ? villainStats(v) : null;
                  return (
                    <div className="seat" key={i}>
                      <span className="seat-n">V{i + 1}</span>
                      <select value={s.villainId || ""} onChange={(e) => setSeats((p) => p.map((x, j) => j === i ? { ...x, villainId: e.target.value || null } : x))}>
                        <option value="">unknown player</option>
                        {villains.map((v2) => <option key={v2.id} value={v2.id}>{v2.name}</option>)}
                      </select>
                      <select value={s.action} onChange={(e) => setSeats((p) => p.map((x, j) => j === i ? { ...x, action: e.target.value } : x))}>
                        <option value="none">no action</option>
                        <option value="call">called</option>
                        <option value="bet">bet</option>
                        <option value="raise">raised</option>
                        <option value="3bet">3-bet</option>
                      </select>
                      <span className="seat-r">{(opponents[i]?.pct * 100 || 0).toFixed(0)}%</span>
                      {st && <span className={`tag t-${st.tag}`}>{st.tag}</span>}
                    </div>
                  );
                })}
              </div>
              <p className="hint">The % is how wide a range that seat is being given. Tracked opponents use their own stats; unknown players use a typical 26%.</p>
            </div>
...
.stop{background:var(--panel2);border:0;padding:10px 8px;text-align:left;display:flex;flex-direction:column;gap:3px}
