...
// Display buckets add a 10th slot so a royal is never reported as a plain straight flush.
const DIST_NAMES = [...CAT_NAMES, "Royal flush"];
const isRoyal = (score) => Math.floor(score / Math.pow(15, 5)) === CAT.SF
  && Math.floor(score / Math.pow(15, 4)) % 15 - 2 === 12;
...
/* `polarise` is how much meaning to read into their aggression. Against a
   thinking player a bet really does narrow their range (polarise = 1). Against
   someone who bets whatever they have, it barely narrows it at all, and
   pretending otherwise makes you fold far too much. */
const ACTION_SHAPE = {
  bet: { exp: 3.4, floor: 0.095 },
  raise: { exp: 5, floor: 0.07 },
  "3bet": { exp: 7, floor: 0.05 },
};
function actionWeight(action, polarise = 1) {
  const p = Math.max(0, Math.min(1, polarise));
  if (action === "call") {
    const spread = 0.0648 / Math.max(0.25, p);
    return (s) => Math.exp(-Math.pow(s - 0.6, 2) / spread) + 0.05 + (1 - p) * 0.6;
  }
  const sh = ACTION_SHAPE[action];
  if (!sh) return null;
  const e = 1 + (sh.exp - 1) * p;          // p=0 -> flat, p=1 -> fully polarised
  const floor = sh.floor + (1 - p) * 0.55; // flat floor keeps their air in range
  return (s) => Math.pow(s, e) + floor;
}

function buildRange(dead, board, pct, action, polarise = 1) {
  const pool = [];
  ...
  const f = actionWeight(action, polarise);
  if (board.length < 3 || !f) {
...
function createSim(hero, board, opponents, seed, oneCard, polarise = 1) {
  ...
  const pools = opponents.map((o) => {
    const r = buildRange(dead, board, Math.max(0.02, Math.min(1, o.pct)), o.action, polarise);
  ...
  const acc = { win: 0, tie: 0, lose: 0, n: 0, dist: new Float64Array(10) };
  const used = new Uint8Array(52);
  const oppHands = new Array(nOpp);
  const runBoard = new Array(5);
  const need = oneCard ? Math.min(1, 5 - board.length) : 5 - board.length;
  ...
  function run(iters) {
    ...
    while (done < iters && guard < iters * 40) {
      ...
      if (!ok) continue;
      runBoard.length = board.length + need;
      ...
      acc.dist[isRoyal(hs) ? 9 : categoryOf(hs)]++;
    ...
    let strong = 0;
    for (let c = CAT.TRIPS; c <= 9; c++) strong += acc.dist[c];
    return {
...
/* If calling now is -EV, this is how many extra chips you must win on later
   streets for the call to break even. Solves eq*(pot+extra) = (1-eq)*toCall. */
function impliedNeeded(eq, pot, toCall) {
  if (eq <= 0) return Infinity;
  return ((1 - eq) * toCall) / eq - pot;
}
...
function sizingOptions({ eq, pot, toCall, stack, wet, nOpp, sizeBias = 0 }) {
  ...
  pick = Math.max(0, Math.min(2, pick + sizeBias));          // style nudge, never forces all-in
...
function rangeForOpponent(v, action, street, assumeVpip) {
  const s = v ? villainStats(v) : { vpip: assumeVpip ?? PRIOR.vpip, conf: 0 };
  let pct = s.vpip;
...
/* Styles shift the parameters where poker genuinely allows a choice: how thin
   you value bet, how often you bluff, how much you expect to collect on later
   streets, how big you size. They are NOT allowed to reverse a decision that is
   clearly right or clearly wrong - see the clamp in decide(). */
const MODES = {
  balanced: {
    label: "Balanced", blurb: "Plays close to the raw maths. The default.",
    callEdge: 0.02, valueBet: 0.62, bluffCeiling: 0.35, sizeBias: 0,
    capture: 0.35, rfiBias: 1, thinValue: 0,
    assumeVpip: 0.26, polarise: 1,
  },
  aggressive: {
    label: "Aggressive", blurb: "More bluffs, thinner value bets, bigger sizing. Best against players who fold too much.",
    callEdge: -0.01, valueBet: 0.55, bluffCeiling: 0.44, sizeBias: 1,
    capture: 0.45, rfiBias: 1.3, thinValue: 0.07,
    assumeVpip: 0.3, polarise: 0.8,
  },
  loose: {
    label: "Loose table", blurb: "For players who see every flop, call too much and bet whatever they have. Value bet thin, size up, almost never bluff.",
    callEdge: 0.015, valueBet: 0.5, bluffCeiling: 0.1, sizeBias: 1,
    capture: 0.55, rfiBias: 1.05, thinValue: -0.06,
    assumeVpip: 0.45, polarise: 0.35,
  },
  cautious: {
    label: "Cautious", blurb: "Fewer bluffs, only clear spots, smaller sizing. Best at a loose table where you get paid anyway.",
    callEdge: 0.055, valueBet: 0.7, bluffCeiling: 0.2, sizeBias: -1,
    capture: 0.25, rfiBias: 0.72, thinValue: -0.05,
    assumeVpip: 0.22, polarise: 1,
  },
};
...
/* Pure so we can evaluate any style, including the balanced baseline used
   for the "your style changed this" note. */
function decide(c, m) {
  const { eqNow, eq, req, ev, toCall, pot, potBefore, texture, spr, street,
          prePct, preCode, preRank, position, bb, nOppActive, usingOneCard,
          eqOne, implied, behind } = c;
  const edge = eqNow - req;
  const wet = texture ? texture.wet : 0;
  const impliedRescues = usingOneCard && implied > 0 && implied <= behind * m.capture;
  const reasons = [];
  let action, tone;

  if (toCall <= 0) {
    if (eqNow > m.valueBet) {
      action = "BET FOR VALUE"; tone = "good";
      reasons.push(`You're ahead ${(eqNow * 100).toFixed(1)}% of the time \u2014 worse hands can still pay you.`);
      reasons.push(wet > 55 ? "Wet board: size up to charge draws." : "Dry board: a smaller bet keeps their weak hands in.");
      if (m.thinValue > 0 && eqNow < 0.62) reasons.push("This is a thin value bet \u2014 it only works against someone who calls too wide.");
      if (m.polarise < 0.5 && eqNow < 0.62) reasons.push("Thin, but they call with worse constantly. Bet it, and bet bigger than feels comfortable.");
    } else if (eqNow > 0.45 + m.thinValue) {
      action = "CHECK"; tone = "neutral";
      ...
      const canBluff = nOppActive === 1 && fe <= m.bluffCeiling;
      action = canBluff ? "BLUFF" : "CHECK / FOLD";
      tone = canBluff ? "neutral" : "bad";
      reasons.push(`Low equity (${(eqNow * 100).toFixed(1)}%). A 60%-pot bluff needs them to fold ${(fe * 100).toFixed(0)}% of the time.`);
      if (canBluff) reasons.push("You have little to lose by checking, so this is a spot where pressure pays.");
      if (!canBluff && m.bluffCeiling <= 0.12 && nOppActive === 1) reasons.push("Bluffing is off the table against this type \u2014 they call. Give up cheaply and wait for a hand.");
      if (nOppActive > 1) reasons.push(`${nOppActive} opponents \u2014 bluffs need every one of them to fold, which compounds against you.`);
    }
  } else {
    if (edge > 0.15 && eqNow > 0.6) {
      action = "RAISE"; tone = "good";
      reasons.push(`Equity ${(eqNow * 100).toFixed(1)}% vs ${(req * 100).toFixed(1)}% needed \u2014 a big edge. Calling leaves money behind.`);
    } else if (edge > m.callEdge) {
      action = "CALL"; tone = "good";
      reasons.push(`You need ${(req * 100).toFixed(1)}% to break even and you have ${(eqNow * 100).toFixed(1)}%.`);
      reasons.push(`Each call like this is worth ${ev >= 0 ? "+" : ""}${(ev / bb).toFixed(2)} bb on average.`);
    } else if (edge > m.callEdge - 0.06) {
      action = "MARGINAL \u2014 CALL OR FOLD"; tone = "neutral";
    ...
    } else if (impliedRescues) {
      action = "CALL \u2014 ON IMPLIED ODDS"; tone = "neutral";
      reasons.push(`On this street alone you're short: ${(eqNow * 100).toFixed(1)}% against ${(req * 100).toFixed(1)}% needed.`);
      reasons.push(`But you only need ${fmt(implied)} more when you hit, and there is ${fmt(behind)} behind.`);
      reasons.push("This depends on actually getting paid \u2014 against someone who folds easily, fold instead.");
    } else {
      action = "FOLD"; tone = "bad";
      reasons.push(`You need ${(req * 100).toFixed(1)}% but only have ${(eqNow * 100).toFixed(1)}%.`);
      reasons.push(`Calling costs ${(ev / bb).toFixed(2)} bb every time you do it.`);
      if (usingOneCard && implied > 0) reasons.push(`Even counting later streets you'd need ${fmt(implied)} more, which is too much against the ${fmt(behind)} behind.`);
    }

    /* CLAMP: no style may fold a clearly profitable call or call a clearly
       losing one. Style breaks ties; it does not overrule the arithmetic. */
    if (edge > 0.1 && action === "FOLD") {
      action = "CALL"; tone = "good";
      reasons.length = 0;
      reasons.push(`Too profitable to fold whatever your style: ${(eqNow * 100).toFixed(1)}% against ${(req * 100).toFixed(1)}% needed.`);
    }
    if (edge < -0.12 && !impliedRescues && (action === "CALL" || action === "RAISE")) {
      action = "FOLD"; tone = "bad";
      reasons.length = 0;
      reasons.push(`Losing too much to justify at any style: ${(eqNow * 100).toFixed(1)}% against ${(req * 100).toFixed(1)}% needed.`);
    }
  }

  if (street === 0 && prePct !== null) {
    const thr = RFI[position] * m.rfiBias;
    if (toCall <= 0 || toCall <= bb) {
      reasons.unshift(prePct <= thr
        ? `${preCode} is in the top ${(prePct * 100).toFixed(0)}% \u2014 inside a ${m.label.toLowerCase()} ${position} opening range (${(thr * 100).toFixed(0)}%).`
        : `${preCode} is top ${(prePct * 100).toFixed(0)}%, outside a ${m.label.toLowerCase()} ${position} open (${(thr * 100).toFixed(0)}%). Folding is fine.`);
    } else {
      reasons.unshift(`${preCode} ranks #${preRank} of 169 (top ${(prePct * 100).toFixed(0)}%).`);
    }
  }
  if (m.polarise < 0.5 && toCall > 0) reasons.push("Their bet barely narrows their range here \u2014 they'd bet this with almost anything, so don't give it much credit.");
  if (usingOneCard) reasons.push(`Calling here only buys the turn, so the number that matters is ${(eqOne * 100).toFixed(1)}%, not the ${(eq * 100).toFixed(1)}% you'd have if all the money went in now.`);
  if (texture && texture.wet > 60 && street < 3) reasons.push(`Board is wet (${texture.wet}/100) \u2014 your equity is fragile on later cards.`);
  if (spr > 0 && spr < 1.5 && eqNow > 0.5) reasons.push(`SPR ${spr.toFixed(1)} \u2014 this is a commitment spot; plan to get it all in.`);
  return { action, tone, reasons, edge, impliedRescues };
}
...
export default function PokerDesk() {
  ...
  const [simOne, setSimOne] = useState(null);
  ...
  useEffect(() => {
    if (!loaded) return;
    store.set("poker:data", { villains, sessions, hands, sigma, mode });
  }, [villains, sessions, hands, sigma, mode, loaded]);
  ...
  const opponents = useMemo(
    () => seats.filter((s) => !s.folded).map((s) => ({
      pct: rangeForOpponent(villains.find((v) => v.id === s.villainId), s.action, street, MODES[mode].assumeVpip),
      action: s.action,
    })),
    [seats, villains, street, mode]
  );
  ...
  useEffect(() => {
    ...
    if (!ready) { setSim(null); setSimOne(null); return; }
    setScan(null);
    const seed = 0x9e37 + street * 7717 + (hole[0] || 0) * 131;
    const pol = MODES[mode].polarise;
    const engine = createSim(hole, knownBoard, opponents, seed, false, pol);
    // On the flop, raw equity assumes you see BOTH remaining cards for free.
    // That is only true all-in, so we also run a one-card-to-come simulation.
    const twoToCome = knownBoard.length === 3;
    const engineOne = twoToCome ? createSim(hole, knownBoard, opponents, seed + 5, true, pol) : null;
    ...
    const tick = () => {
      ...
      setSim(acc);
      if (engineOne && acc.n <= 40000) setSimOne(engineOne.run(SLICE / 2));
      else if (!engineOne) setSimOne(null);
      if (acc.n < TARGET) setTimeout(tick, 0);
    };
    setTimeout(tick, 0);
    return () => { simRef.current++; };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [hole[0], hole[1], board.join(","), street, JSON.stringify(opponents), ready, mode]);

  /* ---- derived math ---- */
  const eq = sim ? sim.equity : 0;
  const eqOne = simOne ? simOne.equity : null;
  const behind = Math.max(0, stack - toCall);
  // If there is money left to play for, the immediate call only buys ONE card.
  const usingOneCard = eqOne !== null && behind > 0;
  const eqNow = usingOneCard ? eqOne : eq;
  const req = requiredEquity(pot, toCall);
  const implied = toCall > 0 ? impliedNeeded(eqNow, pot, toCall) : 0;
  // You rarely win the whole remaining stack when you hit. A third of it is a
  // realistic expectation against a normal opponent.
  const impliedRescues = usingOneCard && implied > 0 && implied <= behind * MODES[mode].capture;
  const ev = evCall(eqNow, pot, toCall);
  ...
  const sizing = useMemo(() => {
    ...
    return sizingOptions({
      eq: eqNow, pot, toCall, stack,
      wet: texture ? texture.wet : 25,
      nOpp: nOppActive,
      sizeBias: MODES[mode].sizeBias,
    });
  }, [sim, eqNow, pot, toCall, stack, texture, nOppActive, mode]);

  const decisionCtx = useMemo(() => ({
    eqNow, eq, req, ev, toCall, pot, potBefore, texture, spr, street,
    prePct, preCode, preRank, position, bb, nOppActive, usingOneCard,
    eqOne, implied, behind,
  }), [eqNow, eq, req, ev, toCall, pot, potBefore, texture, spr, street,
       prePct, preCode, preRank, position, bb, nOppActive, usingOneCard, eqOne, implied, behind]);

  const verdict = useMemo(() => (sim ? decide(decisionCtx, MODES[mode]) : null), [sim, decisionCtx, mode]);
  const baseline = useMemo(
    () => (sim && mode !== "balanced" ? decide(decisionCtx, MODES.balanced) : null),
    [sim, decisionCtx, mode]
  );
  ...
  const runScan = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      for (let c = 0; c < 52; c++) {
        ...
        const s = createSim(hole, [...knownBoard, c], opponents, 555 + c * 31, false, MODES[mode].polarise);
  ...
  }, [ready, street, hole, knownBoard, opponents, mode]);
            ...
            <div className="modes">
              <div className="modebtns">
                {Object.entries(MODES).map(([k, m]) => (
                  <button key={k} className={mode === k ? "on" : ""} onClick={() => setMode(k)}>{m.label}</button>
                ))}
              </div>
              ...
              <>
                ...
                <div className="line-wrap">
                  <div className="line-head">
                    <div>
                      <div className="eq">{(eqNow * 100).toFixed(1)}<em>%</em></div>
                      <div className="eq-l">
                        {usingOneCard ? "equity seeing one card" : "your equity"}
                        {" "}<span className="se">±{((usingOneCard && simOne ? simOne.se : sim.se) * 196).toFixed(2)}</span>
                      </div>
                      {usingOneCard && (
                        <div className="eq-alt">
                          {(eq * 100).toFixed(1)}% if all the money goes in now
                        </div>
                      )}
                    </div>
                  ...
                  {usingOneCard && <div className="line-note">bar shows the all-in split (both cards to come)</div>}
                ...
                {verdict && (
                  <div className={`verdict v-${verdict.tone}`}>
                    ...
                    {baseline && baseline.action !== verdict.action && (
                      <div className="diverge">
                        Your <b>{MODES[mode].label.toLowerCase()}</b> setting changed this.
                        Balanced would say <b>{baseline.action.toLowerCase()}</b>.
                      </div>
                    )}
                  </div>
                )}
                ...
                <div className="stats">
                  ...
                  <Stat label="Your edge" value={`${eqNow - req >= 0 ? "+" : ""}${((eqNow - req) * 100).toFixed(1)}%`}
                    tone={eqNow - req >= 0 ? "good" : "bad"} sub="equity − required" />
                  ...
                  {usingOneCard && toCall > 0 && implied > 0 ? (
                    <Stat label="Need to win later" value={fmt(implied)}
                      tone={impliedRescues ? "good" : "bad"}
                      sub={`${fmt(behind)} left behind`} />
                  ) : (
...
.eq-alt{font-size:10px;color:#6A7080;font-family:var(--mono);margin-top:5px}
.line-note{font-size:9px;color:#5A6070;font-family:var(--mono);margin-top:8px;letter-spacing:0.03em}
...
.modes{background:var(--ground);border:1px solid var(--edge);border-radius:3px;padding:11px 13px}
.modebtns{display:flex;gap:3px}
.modebtns button{flex:1;background:var(--panel2);border:1px solid var(--edge);color:var(--dim);
  font-size:11px;font-weight:600;padding:7px 4px;border-radius:2px;letter-spacing:0.02em}
.modebtns button.on{background:var(--brass);border-color:var(--brass);color:#14161C}
.modes p{font-size:10px;color:#6A7080;margin:8px 0 0;line-height:1.5;font-family:var(--mono)}
.diverge{margin-top:10px;padding-top:9px;border-top:1px dashed var(--edge);
  font-size:11px;color:#8E8A80;line-height:1.5}
.diverge b{color:var(--brass);font-weight:600}
